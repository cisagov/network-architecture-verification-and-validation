...
COL_NAMES = ['Count', 'Src_IP', 'Src_Desc', 'Dest_IP', 'Dest_Desc', 'Port', 'Service', 'Proto', 'Conn_State', 'Notes']
...
IPV6_CELL_COLOR = (openpyxl.styles.PatternFill('solid', fgColor='FFFFFF'),
                   openpyxl.styles.Font(name='Calibri', size=11, color='ff0000'))
EXTERNAL_NETWORK_CELL_COLOR = (openpyxl.styles.PatternFill('solid', fgColor='030303'),
                               openpyxl.styles.Font(name='Calibri', size=11, color='ffff00'))
INTERNAL_NETWORK_CELL_COLOR = (openpyxl.styles.PatternFill('solid', fgColor='ffff00'),
                               openpyxl.styles.Font(name='Calibri', size=11, color='000000'))
ICMP_CELL_COLOR = (openpyxl.styles.PatternFill('solid', fgColor='ff33cc'),
                   openpyxl.styles.Font(name='Calibri', size=11, color='000000'))
UNKNOWN_EXTERNAL_CELL_COLOR = (openpyxl.styles.PatternFill('solid', fgColor='ffffff'),
                               openpyxl.styles.Font(name='Calibri', size=11, color='000000'))
...
def get_segments_data(ws):
    ...
    for row in itertools.islice(ws.iter_rows(), 1, None):
        if not row[2].value:
            continue
        segments.append(data_types.Segment(
            name=row[0].value,
...
def create_analysis_array(sort_input, **kwargs):
    ...
    counted = sorted(list(str(count) + '\t' + item for item, count in
                     sorted(Counter(sort_input).items(), key=lambda x: x[0])),
                     key=lambda x: int(x.split('\t')[0]),
                     reverse=True)
    for row in counted:
        cells = row.split('\t')
        arr.append(data_types.AnalysisRowItem(count=cells[0], src_ip=cells[1], dest_ip=cells[2], port=cells[3], proto=cells[4], conn=cells[5]))
...
@utilities.timeit
def perform_analysis(wb, rows, services, conn_states, inventory, segments, dns_data, pkl_path, ext_IPs, unk_int_IPs, **kwargs):
    sheet = make_sheet(wb, 'Analysis', idx=0)
    sheet.append(['Count', 'Src_IP', 'Src_Desc', 'Dest_IP', 'Dest_Desc', 'Port', 'Service', 'Proto', 'Conn_State', 'Notes'])
    ...
    if netaddr.valid_ipv6(ip_to_check) or netaddr.IPAddress(ip_to_check).is_multicast():
        ...
        for segment in segments[:-1]:
                ...
                if ip_to_check in dns_data:
                    ...
                    desc_to_change = (f'Unknown device in {segment.name} network', segment.color)
...
def auto_adjust_width(sheet):
    ...
    for col in sheet.columns:
        vals = (len(u"{}".format(c.value)) for c in col if c.value is not None)
